...
// this maybe obselete in new nodejs (with async/await)
process.on('uncaughtException', async function(err) {
  ...
  try {
    await addExceptionAsLastEntryToLog(err);
...
async function addExceptionAsLastEntryToLog(err) {
  ...
  if(driverAndActionsDI.isWebDriverActive()) {
    return driverAndActionsDI.takeScreenshot(async function(err, base64data) {
...
module.exports = async function(targetToTest) {
  return new Promise(async (resolve) => {
    try {
      ...
      //endregion
      console.log("ASASS", targetToTest);
