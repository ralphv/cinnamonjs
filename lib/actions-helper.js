...
module.exports = {
  ...
  setImplicitWait: function(driver, timeout) {
    ...
    //return driver.manage().timeouts().implicitlyWait(timeout);
  ...
  restoreImplicitWait: function(driver) {
    return this.setImplicitWait(driver, this.implicitWaitTimeout);
  },
  findElementsAndWait: function(driver, locator, timeout) {
    ...
    return self.setImplicitWait(driver, timeout).then(function() {
  ...
  openFileWithDefaultApp: function(file) {
    /^win/.test(process.platform) ? require("child_process").exec('start "" "' + file + '"')
      : require("child_process").spawn(getCommandLine(), [file], {detached: true});
  }
};
