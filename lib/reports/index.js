...
const self = module.exports = {
  createReports: async function(log) {
    // save result file.
    const now = new Date();
    let basePath = now.getHours() + timeSeparator + now.getMinutes() + timeSeparator + now.getSeconds() + "." + now.getMilliseconds() + " " + now.getFullYear() + "-" + now.getMonth() + "-" + now.getDay();
    basePath = path.join(config.reportsOutputFolder, basePath, path.sep);
    self.mkdir(basePath);
  ...
  mkdir: function(folder) {
    let index = folder.indexOf(path.sep, 1);
    while(index !== -1) {
      ...
      index = folder.indexOf(path.sep, index + 1);
    }
  },
  namesToFiles: {
    "json": "./json.js",
